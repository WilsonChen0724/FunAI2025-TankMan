...
def parser_arg() -> Namespace:
    ...
    parser.add_argument("--total-time-steps", type=int, default=10000000) #調總共要跑幾個step
    parser.add_argument("--n-envs", type=int, default=6)

    # PPO Hyperparameters
    parser.add_argument("--hidden-sizes", type=int, nargs="*", default=[64, 64])
    parser.add_argument("--batch-size", type=int, default=300)
    parser.add_argument("--step-per-update", type=int, default=300) #每個update要跑幾個step
...
def train(opts: Namespace) -> None:
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=opts.step_per_update * opts.n_envs * 5,
